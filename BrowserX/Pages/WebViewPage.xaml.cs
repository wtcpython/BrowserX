...
public sealed partial class WebViewPage : Page
{
    ...
    public WebViewPage(Uri WebUri)
    {
        ...
        FavoriteList.Visibility = App.Profile.MenuStatus == "Always" ? Visibility.Visible : Visibility.Collapsed;
    }
    ...
    {
        HomeButton.Visibility = App.Profile.ToolBar!["HomeButton"] ? Visibility.Visible : Visibility.Collapsed;
    }
    ...
    private async void InitializeWebView2Async(WebView2 WebView, Uri WebUri)
    {
        ...
        if (WebUri != null && WebUri != WebView.Source) WebView.Source = WebUri;
    ...
    {
        if (ProfileHelper.Favorites.Any(x => x.Uri.Equals(sender.Source)))
            InFavoriteList = true;
        else
    ...
    private void CoreWebView2_NavigationCompleted(CoreWebView2 sender, CoreWebView2NavigationCompletedEventArgs args)
    {
        ...
        {
            if (!CachedIconSource.TryGetValue(sender.FaviconUri, out var iconSource))
            {
    ...
    {
        if (!NavigationCompleted || sender.FaviconUri.Length == 0) return;
        var faviconUri = new Uri(sender.FaviconUri);
        if (!CachedIconSource.TryGetValue(sender.FaviconUri, out var iconSource))
        {
            iconSource = new ImageIconSource
            {
        ...
        tabViewItem.IconSource = iconSource;
        var history = App.Histories.FirstOrDefault(x => x.NavigationId == NowNavigationId, null);
        history?.FaviconUri = faviconUri;
        var favorite = ProfileHelper.Favorites.FirstOrDefault(x => !x.CustomIcon && x.Uri.Equals(faviconUri), null);
    ...
    {
        var tabView = WindowHelper.GetWindowForElement(this)?.Content as TabView;
        tabView?.TabItems.Remove(tabViewItem);
    }

    private async void CoreWebView2_ScriptDialogOpening(CoreWebView2 sender,
        CoreWebView2ScriptDialogOpeningEventArgs args)
    {
        var title = $"{new Uri(args.Uri).Host} 显示";
        ...
        else if (args.Kind == CoreWebView2ScriptDialogKind.Confirm)
        {
            ...
            if (result == ContentDialogResult.Primary) args.Accept();
        }
    ...
    private void CoreWebView2_ContextMenuRequested(UIElement element, CoreWebView2 sender,
    ...
    private void CreateMenuItems(CoreWebView2ContextMenuRequestedEventArgs args,
        IList<CoreWebView2ContextMenuItem> webMenuItems, IList<MenuFlyoutItemBase> menuItems)
    {
    ...
    private MenuFlyoutSubItem CreateSubMenuItem(CoreWebView2ContextMenuRequestedEventArgs args,
        CoreWebView2ContextMenuItem menuItem)
    {
    ...
    private static MenuFlyoutItem CreateCommandMenuItem(CoreWebView2ContextMenuRequestedEventArgs args,
        CoreWebView2ContextMenuItem menuItem)
    {
        ...
        System.Threading.SynchronizationContext.Current?.Post(async (_) =>
        {
            using (deferral)
            {
                ...
                if (result != null)
                {
                    ...
                    if (App.Profile.ShowFlyoutWhenStartDownloading) ShowFlyout("下载");
                }
                else
                    args.Cancel = true;
            }
    ...
    {
        var info = ProfileHelper.Favorites.FirstOrDefault(x => x.Uri.Equals(WebViewEngine.Source));
        if (info != null)
        {
        ...
        else
        {
            WebsiteInfo newInfo = new()
            {
                ...
                Icon = WebViewEngine.CoreWebView2.FaviconUri.Length > 0
                    ? new Uri(WebViewEngine.CoreWebView2.FaviconUri)
                    : null,
                Uri = WebViewEngine.Source
            };
            ProfileHelper.Favorites.Add(newInfo);
            InFavoriteList = true;
        }
    ...
    {
        FavoriteButton.IconGlyph = InFavoriteList ? "\ue735" : "\ue734";
    }
