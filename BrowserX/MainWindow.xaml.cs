...
public sealed partial class MainWindow : Window, INotifyPropertyChanged
{
    ...
    public MainWindow()
    {
        ...
        mainWindowSubClassProc = MainWindowSubClassProc;
        PInvoke.SetWindowSubclass(new HWND(this.GetWindowHandle()), mainWindowSubClassProc, 0, 0);
    }

    public void AddHomePage()
    {
        var uri = App.Profile.SpecificUri;
    ...
    public void AddNewTab(object content, string header = "正在加载…", IconSource? icon = null, int index = -1)
    {
        ...
        var insertIndex = index >= 0 ? index : tabView.TabItems.Count;
    ...
    private void OnAppWindowChanged(AppWindow sender, AppWindowChangedEventArgs args)
    {
        ...
        {
            if (TitlebarMenuFlyout.IsOpen) TitlebarMenuFlyout.Hide();
    ...
    {
        PInvoke.SendMessage(new HWND(this.GetWindowHandle()), PInvoke.WM_SYSCOMMAND, 0xF010, 0);
    }
    ...
    {
        PInvoke.SendMessage(new HWND(this.GetWindowHandle()), PInvoke.WM_SYSCOMMAND, 0xF000, 0);
    }
    ...
    /// 应用主窗口消息处理
    /// </summary>
    private LRESULT MainWindowSubClassProc(HWND hWnd, uint Msg, WPARAM wParam, LPARAM lParam, nuint uIdSubclass,
        nuint dwRefData)
    {
        switch (Msg)
        {
    ...
    {
        var webView2 = WebViewHelper.GetWebView(tabView.SelectedItem as TabViewItem);
        webView2?.Reload();
    }
    ...
    {
        var item = e is TabViewTabCloseRequestedEventArgs args ? args.Tab : tabView.SelectedItem as TabViewItem;
        if (item?.Content is WebViewPage page) page.Close();

        tabView.TabItems.Remove(item);

        if (tabView.TabItems.Count <= 0)
            Close();
        else if (App.NeedRestartEnvironment && !WebViewHelper.AnyWebviewPageExists()) App.WebView2.Close();
    }
    ...
    {
        while (tabView.TabItems.Count > tabView.SelectedIndex + 1) tabView.TabItems.RemoveAt(tabView.SelectedIndex + 1);
    ...
    private void TabViewPointerPressed(object sender, PointerRoutedEventArgs e)
    {
        ...
        if (properties.IsMiddleButtonPressed)
        {
            ...
            foreach (var item in tabView.TabItems)
                if (item is TabViewItem tabViewItem)
                {
                    ...
                    var itemBounds =
                        transform.TransformBounds(new Rect(0, 0, tabViewItem.ActualWidth, tabViewItem.ActualHeight));
